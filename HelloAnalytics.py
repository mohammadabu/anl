...
def save_page_view_report(data,VIEW_ID,day):
  ...
  if(data):
    for x in data:
      cursor = connection.cursor()
      pg_insert = """ INSERT INTO consultancy_integrations."ga_page_view" (top_level, sub_folder, entity_id,community_id,community_name,users,pageviews,date)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s)"""
      inserted_values = (VIEW_ID[0], VIEW_ID[1], VIEW_ID[2], VIEW_ID[3], VIEW_ID[4],data[x]['metric']['ga:users'],data[x]['metric']['ga:pageviews'],day_norway)
      cursor.execute(pg_insert, inserted_values)
      connection.commit()
...
def save_device_report(data,VIEW_ID,day):
  ...
  if(data):
    for x in data:
      cursor = connection.cursor()
      pg_insert = """ INSERT INTO consultancy_integrations."ga_device" (top_level, sub_folder, entity_id,community_id,community_name,users,mobiledevicebranding,date)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s)"""
      inserted_values = (VIEW_ID[0], VIEW_ID[1], VIEW_ID[2], VIEW_ID[3], VIEW_ID[4],data[x]['metric']['ga:users'],data[x]['dimension']['ga:mobileDeviceBranding'],day_norway)
      cursor.execute(pg_insert, inserted_values)
      connection.commit()
  ...
  return analytics.reports().batchGet(
      body={
        'reportRequests': [
        {
          ...
          'dateRanges': [{'startDate': str(day), 'endDate': str(day)}],
          'metrics': [{'expression':'ga:users'}],
          'dimensions': [{'name':'ga:mobileDeviceBranding'}],
        }]
      }
  ).execute()
